...
contract EstateAgent is Administration {
    ...
    event SetToken(RealestateToken newContract);
    ...
    constructor(uint256 currentLimit, uint256 basePrice) public{
        token = new RealestateToken(address(this));
     ...
     * @param _newContract the address of the newly deployed PROPERTY token
     * In case if token address ever changes, we can set this contract to point there
     */
    function setToken(RealestateToken _newContract) external onlyAdmin {
        token = _newContract;
