...
contract("EstateAgent", function(accounts) {
    ...
    it("Set new token contract", async function() {
        ...
        await estateAgentTokenInstance.setToken(this.realestateTokenInstance.address, { from: admin });
        expect(await estateAgentTokenInstance.token.call({ from: admin })).to.be.equal(this.realestateTokenInstance.address);
    });
