...
contract("RentalAgent", function(accounts) {
    ...
    beforeEach(async function() {
        ...
        realestateToken = await RealestateToken.at(
            await estateAgent.token({ from: admin })
        );
        rentalAgentTokenInstance = await RentalAgent.new(realestateToken.address, estateAgent.address);
    });
    ...
    it("Testing deposit() function", async function() {
        ...
        await realestateToken.approve(rentalAgentTokenInstance.address, tokenId, { from: purchaser });
    ...
    it("Testing withdrawProperty() function", async function() {
        ...
        const tokenId = tx.logs[0].args[2].toString();
        let ownerBefore = await realestateToken.ownerOf(tokenId, { from: newPurchaser });

        //Approve the transfer        
        await realestateToken.approve(rentalAgentTokenInstance.address, tokenId, { from: newPurchaser });

        //Now deposit
        await rentalAgentTokenInstance.deposit(tokenId, { from: newPurchaser });

        //Withdraw
        await rentalAgentTokenInstance.withdrawProperty(tokenId, { from: newPurchaser });
        let ownerAfter = await realestateToken.ownerOf(tokenId, { from: newPurchaser });
    ...
    it("Set new token contract", async function() {
        const newToken = await RealestateToken.new(agent);
